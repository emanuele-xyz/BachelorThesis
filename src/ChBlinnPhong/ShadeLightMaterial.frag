...
vec3 ambient = inLightMaterialAmbient * inObjectMaterialAmbient;

// Compute diffuse lighting
vec3 normal = normalize(inNormal);
vec3 lightDirection =
    normalize(inLightPosition - inFragmentPosition);
float diffuseImpact = max(dot(normal, lightDirection), 0.0);
vec3 diffuse =
    inLightMaterialDiffuse * (diffuseImpact * inObjectMaterialDiffuse);

// Compute specular lighting
float blinnShininessScale = 3;
vec3 cameraDirection =
    normalize(inCameraPosition - inFragmentPosition);
...
vec3 specular =
    inLightMaterialSpecular * (specularImpact * inObjectMaterialSpecular);
